...
def rule_gen(file, outfile):
    ...
    for i in range(len(lines)):
        lines[i] = str.strip(lines[i])
    res_ip = ip_query_gen(file)
    res_domain = domain_query_gen(file)
    if bool(res_ip):
        rule_template = '{"rule_id": "${rule_id}", "risk_score": ${risk_score}, "description": "Auto generated by tools", "interval": "", "name": "${name}", "severity": "",  "tags": [ "ip", "malware", "auto-gen ],  "type": "query",  "from": "now-5m",   "query": "${query}",  "language": "kuery",  "filters": [],  "enabled": false}'
    ...
    if bool(res_domain):
        rule_template = '{"rule_id": "${rule_id}", "risk_score": ${risk_score}, "description": "Auto generated by tools", "interval": "", "name": "${name}", "severity": "",  "tags": [ "domain", "malware", "auto-gen ],  "type": "query",  "from": "now-5m",   "query": "${query}",  "language": "kuery",  "filters": [],  "enabled": false}'
        t = string.Template(rule_template)
